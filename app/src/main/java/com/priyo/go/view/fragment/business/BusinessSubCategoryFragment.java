...
import static com.priyo.go.view.activity.business.BusinessActivity.ID_FRAGMENT_BUSINESS_LIST;

/**
 * Created by sajid.shahriar on 5/11/17.
 */

public class BusinessSubCategoryFragment extends Fragment implements HttpResponseListener, RecyclerView.OnItemClickListener {
    ...
    public static BusinessSubCategoryFragment newInstance(Bundle bundle) {
        BusinessSubCategoryFragment businessCategoryFragment = new BusinessSubCategoryFragment();
    ...
    public boolean onItemClick(View clickedItemView, int position, long id) {
        if (position >= 0) {
            ...
            bundle.putString("subTitle", categoryNode.getNodeTitle());
            onFragmentInteractionListener.addToBackStack(ID_FRAGMENT_BUSINESS_LIST, bundle);
            return true;
        } else {
            return false;
        }
